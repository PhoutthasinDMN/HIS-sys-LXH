...
function getNextPatientID() {
  try {
    ...
    if (!rows || rows.length === 0) return "LXH0000001";
    var last = rows[0].Patient_ID || "LXH0000000";
    var num = parseInt(last.replace("LXH", ""), 10);
    if (isNaN(num)) return "LXH0000001";
    return "LXH" + ("0000000" + (num + 1)).slice(-7);
  } catch(e) { return "LXH0000001"; }
}
