const SUPABASE_URL = "https://ybifnotznyznuhazktbt.supabase.co";

const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InliaWZub3R6bnl6bnVoYXprdGJ0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM5OTY5NTMsImV4cCI6MjA4OTU3Mjk1M30.uEzlcEy11YSBR0ebH0ithY8_O6QQ7S73-fz7ubndzQE";
...
window.loadView = function (v) {
  ...
  if (v === 'settings') {
    supabaseClient.from('Settings').select('Key,Value').then(({ data }) => {
      let s = { hospitalName: 'Luckxay Hospital', logoUrl: '', opdHeaderUrl: '', opdFooterUrl: '' };
      (data || []).forEach(r => {
...
window.fetchDashboardData = async function () {
  ...
  try {
    ...
    const shift = $('#dashShift').val() || 'all';
    
    const visitsWithDetails = [];
    data.forEach(v => {
      let dObj = new Date(v.Date);
      let mins = dObj.getHours() * 60 + dObj.getMinutes();
      
      if (shift === 'morning' && (mins < 450 || mins > 749)) return;
      if (shift === 'afternoon' && (mins < 750 || mins > 989)) return;
      if (shift === 'evening' && (mins < 990 || mins > 1170)) return;
...
window.exportDashboardPDF = function () {
  ...
  Swal.fire({ 
    title: 'ກຳລັງສ້າງ PDF ແບບຄວາມລະອຽດສູງ...', 
    text: 'ກະລຸນາລໍຖ້າປະມານ 3-5 ວິນາທີເພື່ອໃຫ້ລະບົບປະມວນຜົນຮູບພາບ...', 
    heightAuto: false, 
  ...
  setTimeout(() => {
    // 1. Force absolute scroll zeroing across ALL browsers
    ...
    setTimeout(() => {
      ...
      html2pdf().set(opt).from(element).save().then(() => {
        ...
        element.classList.remove('pdf-export-mode');
        document.body.classList.remove('exporting-pdf');
        Swal.fire('Error', 'Export ຜິດພາດ: ' + err.message, 'error');
      });
    }, 500); 
  }, 300);
};
    ...
    setTimeout(() => {
      const opt = {
        ...
        html2canvas: { 
          ...
          logging: false,
          onclone: (clonedDoc) => {
            const captureEl = clonedDoc.getElementById('reportPrintArea');
            if (captureEl) {
              clonedDoc.body.prepend(captureEl);
              clonedDoc.body.style.display = 'block';
              clonedDoc.body.style.margin = '0';
              clonedDoc.body.style.padding = '0';
              captureEl.style.setProperty('margin', '0 auto', 'important');
              captureEl.style.setProperty('padding', '0', 'important');
            }
          }
        },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }
...
window.generateNextPatientID = async function () {
  try {
    ...
    return 'LXH' + ('0000000' + (maxNum + 1)).slice(-7);
  } catch (err) {
    console.error("Error generating next Patient ID:", err);
    return 'LXH0000001'; // Fallback
  }
};
...
window.loadAppointments = async function () {
  ...
  try {
    ...
    let h = '';
    if (r && r.length > 0) {
      console.log('Appointments data:', r);
      r.forEach(a => {
        console.log('Appointment:', a);
        // Use Appt_ID as the primary key
        let aptId = a.Appt_ID;
        console.log('Found Appt_ID:', aptId, 'Full appointment:', a);
        let bs = '', ac = '';
        if (a.Status === 'Completed') {
          bs = '<span class="badge bg-success">ສຳເລັດ</span>';
          ac = `<button class="btn btn-sm btn-outline-danger shadow-sm" onclick="window.deleteAppt('${aptId}','${a.Patient_ID || ''}','${a.Appt_Date || ''}','${a.Appt_Time || ''}')"><i class="fas fa-trash"></i></button>`;
          ...
          ac = `<button class="btn btn-sm btn-success shadow-sm me-1" onclick="window.completeAppt('${aptId}','${a.Patient_ID || ''}','${a.Appt_Date || ''}','${a.Appt_Time || ''}')"><i class="fas fa-check"></i></button><button class="btn btn-sm btn-dark shadow-sm me-1" onclick="window.missedAppt('${aptId}','${a.Patient_ID || ''}','${a.Appt_Date || ''}','${a.Appt_Time || ''}')"><i class="fas fa-user-slash"></i></button><button class="btn btn-sm btn-secondary shadow-sm" onclick="window.cancelAppt('${aptId}','${a.Patient_ID || ''}','${a.Appt_Date || ''}','${a.Appt_Time || ''}')"><i class="fas fa-times"></i></button>`;
        } else {
    ...
    // Initialize DataTable after HTML is set
    setTimeout(() => {
...
window.submitApptForm = async function (e) {
  ...
  let isEdit = $('#a_id').val() !== '';
  let row = {
    Patient_ID: $('#a_target_id').val(), Patient_Name: $('#a_target_name').val(),
    Appt_Date: $('#a_date').val(), Appt_Time: $('#a_time').val(),
    Type: $('#a_type').val(), Reason: $('#a_reason').val(), Doctor: $('#a_doctor').val()
  };
...
window.deleteAppt = async function (id, patientId, apptDate, apptTime) {
  let aptId = (id === 'null' || id === 'undefined') ? null : id;
  let usePatientId = false;
  
  // Fallback: find by Patient_ID + Date if Appt_ID is null
  if (!aptId && patientId && apptDate) {
    const { data: found } = await supabaseClient
      ...
      .single();
    if (found && found.Appt_ID) aptId = found.Appt_ID;
    else if (found && !found.Appt_ID) {
      usePatientId = true;
    }
  }
  ...
  if (r.isConfirmed) {
    if (aptId) {
      await supabaseClient.from('Appointments').delete().eq('Appt_ID', aptId);
    } else if (usePatientId) {
      let deleteQuery = supabaseClient.from('Appointments').delete().eq('Patient_ID', patientId).eq('Appt_Date', apptDate);
      if (apptTime) deleteQuery = deleteQuery.eq('Appt_Time', apptTime);
      await deleteQuery;
    } else {
      Swal.fire('ຜິດພາດ!', 'ບໍ່ພົບລະຫັດການນັດໝາຍ', 'error');
      return;
    }
    window.loadAppointments();
  }
...
window.completeAppt = async function (id, patientId, apptDate, apptTime) {
  try {
    let aptId = (id === 'null' || id === 'undefined') ? null : id;
    let usePatientId = false;
    ...
    if (!aptId && !usePatientId) {
      Swal.fire('ຜິດພາດ!', 'ບໍ່ພົບລະຫັດການນັດໝາຍ', 'error');
      return;
    }
    
    if (usePatientId) {
      let updateQuery = supabaseClient.from('Appointments').update({ Status: 'Completed' }).eq('Patient_ID', patientId).eq('Appt_Date', apptDate);
      if (apptTime) updateQuery = updateQuery.eq('Appt_Time', apptTime);
      const { error } = await updateQuery;
      if (error) throw error;
    } else {
      const { error } = await supabaseClient.from('Appointments').update({ Status: 'Completed' }).eq('Appt_ID', aptId);
      if (error) throw error;
    }
    Swal.fire('ສຳເລັດ!', 'ການນັດໝາຍສຳເລັດແລ້ວ', 'success');
    window.loadAppointments();
    window.checkAlerts();
  } catch (err) {
...
window.missedAppt = async function (id, patientId, apptDate, apptTime) {
  try {
    let aptId = (id === 'null' || id === 'undefined') ? null : id;
    let usePatientId = false;
    ...
    let r = await Swal.fire({
      ...
      cancelButtonText: 'ຢຸດ'
    });
    if (r.isConfirmed) {
      if (usePatientId) {
        let updateQuery = supabaseClient.from('Appointments').update({ Status: 'Missed' }).eq('Patient_ID', patientId).eq('Appt_Date', apptDate);
        if (apptTime) updateQuery = updateQuery.eq('Appt_Time', apptTime);
        const { error } = await updateQuery;
        if (error) throw error;
      } else {
        const { error } = await supabaseClient.from('Appointments').update({ Status: 'Missed' }).eq('Appt_ID', aptId);
        if (error) throw error;
      }
      Swal.fire('ສຳເລັດ!', 'ຂາດນັດແລ້ວ', 'success');
      window.loadAppointments();
      window.checkAlerts();
    }
  } catch (err) {
...
window.cancelAppt = async function (id, patientId, apptDate, apptTime) {
  try {
    let aptId = (id === 'null' || id === 'undefined') ? null : id;
    let usePatientId = false;
    
    // If Appt_ID is null/undefined, try to find the appointment using Patient_ID + Date
    if (!aptId && patientId && apptDate) {
      ...
      if (found && found.Appt_ID) {
        aptId = found.Appt_ID;
        console.log('Found Appt_ID using Patient_ID + Date:', aptId);
      } else if (found && !found.Appt_ID) {
        // No Appt_ID, we'll delete using Patient_ID + Appt_Date directly
        usePatientId = true;
        console.log('Will delete using Patient_ID + Appt_Date');
      }
    }
    ...
    let r = await Swal.fire({
      ...
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'ຍົກເລີກນັດ',
      cancelButtonText: 'ຢຸດ'
    });
    if (r.isConfirmed) {
      console.log('Cancelling appointment (deleting):', { aptId, patientId, apptDate, apptTime, usePatientId });
      let error;
      if (usePatientId) {
        // Delete using Patient_ID + Appt_Date + Appt_Time when Appt_ID is null
        let deleteQuery = supabaseClient.from('Appointments').delete().eq('Patient_ID', patientId).eq('Appt_Date', apptDate);
        if (apptTime) {
          deleteQuery = deleteQuery.eq('Appt_Time', apptTime);
        }
        const result = await deleteQuery;
        error = result.error;
        console.log('Delete result:', result);
      } else {
        // Delete using Appt_ID
        const result = await supabaseClient.from('Appointments').delete().eq('Appt_ID', aptId);
        error = result.error;
      }
      if (error) throw error;
...
window.initPublicQueueView = async function () {
  ...
  // Set Hospital Name
  $('#tvHospitalName').text(systemSettings.hospitalName || "Luckxay Hospital");
